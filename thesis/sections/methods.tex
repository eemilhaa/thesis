...
I implemented a feature-complete interface using both
...
overcome these issues, to an extent, by utilizing a lower-level, self-made,
...
In general, the design of the map interface was intended to be as simple as possible --
...
as it is often the comparison technique
most intuitive for users to understand when using interactive maps
\parencite{lob2015}.
To visualize the travel times of each isochrone polygon,
I used the cividis colour map \parencite{nun2018}.
This colour map was my choice since it is designed to enable accurate visual
...
Also, it is optimized to work equally well
...
Simplicity was the goal in the technical side of the frontend as well.
...
To minimize development-time errors in the implementation,
I used TypeScript \parencite{typescript} as the language.
TypeScript still transpiles to plain JavaScript,
...
This section documents the additional technical infrastructure
needed to enable and deploy the web map application as
a website accessible via the internet.
Being a web application, and a heavily data-reliant one at that,
...
From the perspective of the interactivity of the application,
the backend should respond as quickly as possible.
Considering the above factors,
I implemented the backend
as an HTTP server serving the TTM data as static files directly from the server-side filesystem.
The server software used was Nginx \parencite{nginx},
as it is designed to be as performant and efficient as possible
\parencite{dej2020, ree2008}.
More sophisticated solutions, such as a database,
...
some general frontend tooling was necessary.
...
To serve the bundled frontend, a server is necessary.
...
For the frontend this was simple,
as the bundled frontend application and the server can both be included in the container image,
meaning the container is a completely self-contained server serving the frontend.
For the backend server harder but back to that later...
