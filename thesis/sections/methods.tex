...
and architecture of the implementation.
It also meant that implementing both a frontend (the map application) and a backend
(a solution for getting data to the map application running on the client) was necessary.
...
\parencite{bec2001} I further focused them only as the system
...
These include clicking the map with a pointing device to produce a static map of the clicked location,
and continuously changing the location by moving the cursor over the map
(referred to as hovering).
This enables comparing two different levels of interactivity:
...
For the quality attributes of the system,
...
the frontend application is constrained to running in a web browser.
