...
This was important as the map presentation was simultaneously an output of the development process
...
\begin{figure}[H]
	...
	\caption{
		...
		While thematically distinct,
		the two components of the study were functionally intertwined
		in reaching the broader goals of the study.
	}
...
The \presentacr{ttm} \parencite{fin2023}
...
A significant component of the \acrshort{ttm} is the \presentacr{ykr}
...
Most importantly, however, the part of the \acrshort{ykr} grid overlapping with
...
\begin{figure}[H]
	...
	\caption{The TTM covers the entire Helsinki region.}
...
time of day and / or walking or cycling speed \parencite{fin2023}.
...
The web map application I developed in this study
used the 2023 version of the \acrshort{ttm},
...
\subsection{Development of the web map application}

\subsubsection{Software requirements}
\label{sec:software req}

Software requirements detail the functionalities and properties
...
and to select different travel modes.
A significant quality attribute to consider is that the application should
be responsive enough for real-time interactivity.
...
The high interactivity necessitated a \acrshort{spa} design in the frontend.
...
These included clicking the map with a pointing device to
produce a static accessibility map of the clicked location
(referred to as \textit{locking} the map)
and continuously changing the mapped location and, as a result, the map by moving the cursor over the map
(referred to as \textit{hovering}).
This enabled comparing two different modes of interaction in the study:
a \textit{locked mode}, i.e. producing a static map by clicking,
and a \textit{hovering mode}, i.e. producing
a highly dynamic, \enquote{live},
...
performance was an essential consideration --
both in the sense of software performance
and the responsiveness of the application as perceived by the user.
The most important consideration here was
the aforementioned hovering mode:
Real-time interaction by hovering the mouse above the map
could mean tens of data requests and map renders
per second.
This had to be considered in both the front and backend of the application.
Quality attributes that were invisible to a user, but still vital,
were that the application can be maintained and deployed,
and that it can meet different real-world usage loads.
If these attributes had been lacking,
the application could not have been brought to the users.
...
was the container orchestration platform \textit{Rahti} provided by the \presentacr{csc}.
Rahti is based on the \textit{OpenShift} platform \parencite{openshift},
which in turn runs on \textit{Kubernetes} \parencite{kubernetes}.
Kubernetes is a container orchestration tool
that allows for declaratively managing clusters
of containerized applications and their accompanying services.
I mention these details because to the application they were constraints:
Both the front and backend implementations had to be completely containerized
as they both were deployed with Kubernetes.
Also, as is clear by now,
the frontend of the application was constrained to running in a web browser.
...
\begin{figure}[H]
	...
	\caption{
		The preprocessing pipeline used in processing and simplifying the TTM
		into a format allowing for real-time cartographic interaction with the data.
		In addition to automating a set of preprocessing steps,
		it allows for the separated testing of different parts.
	}
...
on the Firefox \parencite{firefox} and Chromium \parencite{chromium} web browsers. % TODO
...
the goal being to satisfy the software requirements of the map application
(table \ref{tab:functional requirements}; table \ref{tab:nonfunctional requirements}).
...
for three reasons:
It was the framework I am most familiar with,
it was the most widely used and documented framework,
and also it was the only framework that
...
At the time of writing, the frontend was live at \url{https://traveltimematrix.rahtiapp.fi/}.
...
(section \ref{sec:software req}) meant, in essence,
that the only functionality required of the backend
was to respond to requests with TTM data.
No business logic, state management or data manipulation was necessary.
...
I used Vite \parencite{vite} both as a development-time frontend server
...
I used Nginx \parencite{nginx} as the frontend server in production.

As detailed in section \ref{sec:software req},
the deployment environment for both the back- and frontend
was the OpenShift \parencite{openshift} container orchestration platform.
...
As the deployment tool was, in essence, Kubernetes \parencite{kubernetes},
...
except for three open questions in the general section.
...
observe a line-like area or a set of many locations.
...
The purpose was to gain insight on if, and how, the participants themselves considered the
...
The intention here was to gain insight on the usability of the map interface.
