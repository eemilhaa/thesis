...
The design of the map interface was intended to be as simple as possible --
...
To minimize developer-induced errors in the implementation,
...
So, in order to be deployed, both the front- and backends had to be containerized.
...
Both deployments were based around a Deployment object running a set of replicated server pods.
A pod, in this case, was essentially a singular containerized server (either front- or backend).
In addition, a Service object was needed to expose
the internal deployment network (on which the pods run),
and a Route object was needed to route traffic from the internet to the Service.
As the single deviation from the above, the backend deployment also defined a
PersistentVolumeClaim object for storing the TTM data.
...
For further understanding these terms and concepts,
