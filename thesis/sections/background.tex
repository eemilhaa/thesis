...
To further understand what this means,
...
Also, a \acrshort{spa} manages all application \textit{state},
in other words it records, updates and acts on information about
the condition of the application and its components.
In the system described above \parenfig{spa},
the data could be an example of such state.
The server-side, in comparison, is essentially stateless and contains no business logic.
The main point here is that the client-side of a
\acrshort{spa}-style web map is potentially a highly complex application,
and implementing all the needed functionalities from the ground up would be an enormous task.
Software \textit{libraries}, external modules of reusable code,
are essential in overcoming this.
From the perspective of web mapping, two types of libraries are especially important.
In most cases, the map functionalities are map library.
Especially if an application gets more complex,
a general purpose \acrshort{ui} library,
often called a \textit{framework},
can be beneficial in constructing and managing
the general \acrshort{ui} components and state of the application.
