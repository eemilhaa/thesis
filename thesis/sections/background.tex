...
\subsubsection{Web mapping and relevant technologies}

Web mapping refers to the process of making maps for the web,
with web technologies.
A web map, then, is a map that is transferred via the internet,
usually accessed with a web browser,
and read and used as a web page \parencite{sac2017}.
To further understand these terms, it should first be acknowledged that
...
The information is fetched by a client, from a server --
...
The single most important factor enabling the web's transformation from
a document sharing framework to a software ecosystem
was the introduction and standardization of a client-side scripting language,
\textit{JavaScript} \parencite{cho2014}.
...
turning web pages from documents
into dynamic and increasingly interactive applications.
\parencite{jac2019}.
...
To understand how modern web applications work, some additional insight is necessary.
...
and an \emphasizeacr{api} for its
...
carry out client-server communication
...
websites that are fetched only once from the server,
...
This type of website, or web application,
is called a \emphasizeacr{spa} \parencite{fin2014}.

\acrshort{spa}s can provide complex functionality and a high degree of interactivity,
remaining responsive due to fetching and rendering
only what is necessary after the initial page load.
This is an essential aspect in crafting web-based map interfaces,
since almost all interaction with them results in client-server communication,
for example fetching tiled based maps, or data to be mapped
\parencite{mai2017}.
This is further illustrated in REF,
which compares how a map application implemented as a traditional website (REF)
and as a \acrshort{spa} (REF) responds when a user changes the content of the map.

% This type of approach is essential to web maps. tiles?
