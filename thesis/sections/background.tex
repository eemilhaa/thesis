...
command language, form fill-in, menu selection, and direct manipulation
\parencite{soe2015}.
The first three interaction styles are quite self-explanatory,
but the fourth, direct manipulation, depends on context.
In the case of cartographic interaction,
it refers to direct interactions with the map --
for example when the \textit{pan} operator primitive
is executed by dragging the map with a pointing device \parencite{rot2012}.
...
For example, the aforementioned \textit{pan} operation could just as well
