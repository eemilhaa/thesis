...
\subsection{Maps and cartography}
...
The effects this transformation has on a map vary,
...
also considered a science in its own right \parencite{rob2017b}.
...
and the research on maps was in many cases positivist in nature
...
have continued in the context of optimizing cartographic methods (for example \parencite{yan2023, put2014})
...
\parencite{rot2013a, kra2017, }, also contributing new, novel visualization methods
\parencite{lv2017, kes2021}.
...
\subsection{Cartographic interaction}

\subsubsection{Defining \enquote{cartographic interaction}}
\label{sec:cartographic interaction}
...
blending map use and mapmaking at the conceptual level \parencite{eds2008, rot2015}.
When contextualized within the topics and paradigms of cartographic research,
interactive cartography can be understood as
...
in further manipulating the map \parenfig{map interaction} \parencite{rot2012}.
...
facilitate a dialogue by enabling and responding to manipulation by the map user.
...
Considering this study, one key term is still left unexplained.
If cartographic interaction is a dialogue,
I use the term \textit{real-time cartographic interaction}
in specifying a user-map dialogue that happens with no delay perceived by the user.
In other words, the visible interface response to a user action should be as instant as possible,
meaning that the dialogue happens in real-time.
Defining a delay perceivable to a user is a complex question,
as it is dependent on the task at hand and the interface used \parencite{car1980, shn1984, seo2008}.
Also, unlike its measurement, the perception of time is entirely subjective --
In the end, it is the user carrying out the task that either does or does not
perceive something as instant \parencite{seo2008}.
Still, some general limits of system response time are recognized in \acrshort{hci} research:
0.1 seconds is the limit for a response to be perceived instant,
1.0 second is the limit to avoid interrupting the user’s thought process,
and 10 seconds is the limit after which the user’s attention is lost \parencite{nie1993, mil1968}.
...
potentially leading to complete exclusion from accessing an interface \parencite{duc2018, oks2019}.
Still, it has been shown that cartographic interaction and digital maps can be made inclusive --
for example by optimizing the visual cartographic presentation
specifically for visually impaired users \parencite{oks2019},
or by introducing novel interaction methods such as tactile and audio interaction \parencite{bro2015}.
...
and a means to satisfy many different analytical goals and needs
\parencite{rot2013b, rot2015}.
...
in most contexts where maps are present \parencite{fis2021, mei2019, rot2015, rot2021}.
...
\label{sec:deconstructing map interface}
...
interactive maps and the interaction exchanges they enable,
...
make up interaction exchanges between a map and a map user.
...
it refers to direct interactions with the map \parencite{rot2012}.
...
establishing the technological premise for
the process of interaction as two-way communication \parencite{car1997}.
...
At the software level, a computer program
(often referred to as an \textit{application} in the case of user-facing software)
is ultimately what the user interacts with when using a map interface.
...
The primary purpose of an interactive map is
to provide presentation and an interface capabilities,
in short, it acts as a \acrshort{ui} of
a program or, potentially, a system consisting of multiple programs.
...
or even be handled by an entirely different program \parencite{ric2015}.
...
can be thought to make up an interactive system or map \parencite{ric2015, gah1999}.
This approach to reasoning about the structure of a system is often connected to
\textit{layered software architecture},
but is utilized in other architectural patterns too \parencite{ric2015}. % TODO ref?
...
\presentacr{os}-level platforms are closely tied to
the devices (hardware) they operate on:
For example, a map interface developed for a mobile \acrshort{os}
is expected to be operated mainly on relatively small touchscreens and
with potentially constrained computational capabilities \parencite{hus2016},
...
A software-level platform is, in essence, a piece of software that runs other software \parencite{tiw2010}.
There are innumerably many such platforms but,
...
the platform can be run, often meaning less dependence on particular devices \parencite{tiw2010}.
...
and cannot utilize features that the software platform lacks \parencite{tiw2010}.
...
the web has been, and is, a rapidly changing software ecosystem.
...
from static maps sent as images to
...
potentially sending files that are stored on the computer running the server
\parencite{sac2017}.
...
\begin{figure}[H]
	...
	\caption{
		...
		The construction of the web page and the map happens on the server-side,
...
the change of mapped data could be an example of the application state changing.

Simply put, the client-side of a
...
and to acknowledge the fact that any given accessibility measure is ultimately
...
travel time in relation to a single location \parencite{wei2018}.
...
the \textit{isochrone} \parencite{osu2000}.
Isochrones are lines drawn on a map that
connect points of equal travel time \parenfig{isochrones}.
However, map presentations often highlight
the areas deliniated by isochrones instead of
the isochrones themselves.
Such areas of equal travel time are referred to as \textit{isochrone polygons} from here on.
...
based on a set of factors considered impactful to its accessibility \parencite{kim2019}.
This abstracts a lot of complexity at the presentation stage,
as the result is again a single value per location.  % TODO ref
However, what the mapped value means,
is now a question far less straightforward to answer.
...
Another important consideration in visualizing accessibility is
...
\ref{sec:deconstructing map interface}
already covered the theme from a general perspective.
However, one important aspect to consider
...
analyse access in the form they wish to define it \parenfig{goat}.
...
is ultimately confined to a pre-made dataset \parenfig{chronotrains}.
% In general, 
...
% goat & chronotrains? paj2022

% that location's accessibility.
