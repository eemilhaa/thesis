...
in short it acts as a \acrshort{ui} of a potentially larger system.
User actions
...
hardware, operating system (\acrshort{os}) and software level platforms.
...
A key factor, especially considering interactivity,
is that \acrshort{os} platforms are closely tied to
the devices (hardware) they operate on.
For example, applications developed for most mobile \acrshort{os}s
...
while personal computer (\acrshort{pc}) \acrshort{os}s
...
potentially noticeable computational overhead introduced by having to run the platform
in addition to the application,
and cannot utilize features that the platform might lack.
