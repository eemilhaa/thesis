...
\begin{figure}[H]
	...
	\caption{
		Four different accessibility indexes mapped to showcase how
		accessibility differs when measured based on
		different sets of factors \parencite{kim2019}.
	}
...
cartographic interaction can enable new ways of presenting the phenomenon.
After all, the defining principle of interactivity in map presentations is
...
The options in conceptualizing such a map presentation are vast,
...
However, one key distinction to consider
in the context of interactive accessibility representations
is whether the map is an interface to analytical methods,
or to ready-made data on accessibility.
In the former case, the options in producing and manipulating the accessibility map
can be highly sophisticated,
but a delay in user-map-interaction is introduced by
having to compute the data before it can be mapped.
The latter case can potentially enable faster interaction exchanges,
as the data is ready.
As a tradeoff,
user control can be limited since the presentation is confined to a pre-made dataset.
