...
This is known as \presentacr{hci}.
In addition to the process, \acrshort{hci} is a field of research examining
the function of interactive \presentacr{ui}
and \presentacr{ux}
...
the web as a platform has been, and is, a rapidly changing one.
...
and even extensive web \acrshort{gis} platforms.
...
\presentacr{html}, \presentacr{http}, and the concept of client-server architecture
...
the premise of client-server architechture.
...
for example, rendering a web page based on a received \acrshort{html} file
\parencite{sac2017}.
...
potentially with files that are stored on the computer running the sever
\parencite{sac2017}.

A web consisting only of requests, responses and static web pages, as described above,
is hardly reminiscent of the web today.

Software built \enquote{for the web} refers most often to client-side applications,
that use the web browser as a computational environment.
