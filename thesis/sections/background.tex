...
This provided the basis for \textit{client-side computing} --
the concept that a client could
receive and execute code,
turning web pages into dynamic and increasingly interactive pieces of software
\parencite{jac2019}.
Additional key factors here are the \emphasizeacr{dom} and
\emphasizeacr{ajax}.
...
the key takeaway being that this enables client-server communication
without the need of re-fetching and re-rendering the entire page from the server
\parencite{fin2014}.

This, coupled with further advances such as asynchronous operations enabled



Software built \enquote{for the web} refers most often to client-side applications
that use the web browser as a computational environment \parencite{tai2017}.
Client-side technologies are also the main focus in the context of web-mapping,
since a modern web map is most often a client-side application \parencite{rot2014}.
